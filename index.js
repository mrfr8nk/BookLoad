...
async function generateImageNanoBanana(prompt) {
  ...
  const { data } = await axios.get(url, { responseType: 'arraybuffer', timeout: 25000 });
  const buf = Buffer.from(data);
  console.log(`   └─ 🍌 NanoBanana ${buf.length} bytes`);
  return buf;
}
...
async function generateImage(prompt) {
  ...
  // Primary: OmegaTech NanoBanana Pro
  try {
    return await generateImageNanoBanana(prompt);
  } catch (nbErr) {
    console.warn(`   └─ NanoBanana fallback: ${nbErr.message?.substring(0, 60)}`);
  }
...
// ─── PDF project — shared level context + stage prompts (see project-prompts.js) ─
// Imported at top of file: levelContext, PREAMBLE_PROMPT, STAGE1_PROMPT..STAGE6_PROMPT
// This keeps the WhatsApp bot and web portal project generators in sync.
